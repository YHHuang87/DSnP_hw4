...
public:
   ...
   void reset(size_t b = 0) {
      ...
      _activeBlock->reset();
      MemBlock<T>* b_ptr = _activeBlock->getNextBlock();
      while(b_ptr != 0){
        b_ptr->reset();
        MemBlock<T>** temp = &_activeBlock->_nextBlock;
        b_ptr = b_ptr->getNextBlock();
        *temp = 0;
      }
      if(b != _blockSize && b != 0){
        _blockSize = b;
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      MemRecycleList<T>* n_list = new MemRecycleList<T>(n);
      ptr->_nextList = n_list;
      // TODO
      return n_list;
   }
   ...
   T* getMem(size_t t) {
      ...
      // TODO
      MemRecycleList<T>* rl_ptr = getMemRecycleList(n);
      if(rl_ptr->_first != 0){
        ret = rl_ptr->popFront();
            #ifdef MEM_DEBUG
            cout << "Recycled from _recycleList[" << n << "]..." << ret << endl;
        ...
        if(_activeBlock->getRemainSize() == S){
          rl_ptr = getMemRecycleList(0);
          rl_ptr->pushFront((T*)_activeBlock->_ptr);
        }
        else if(_activeBlock->getRemainSize() > S){
          size_t num = ((_activeBlock->getRemainSize()-SIZE_T)/S);
          rl_ptr = getMemRecycleList(num);
          rl_ptr->pushFront((T*)_activeBlock->_ptr);
        } 
        MemBlock<T>* n_ptr = _activeBlock; 
        _activeBlock = new MemBlock<T>(n_ptr,_blockSize);
        //cout << (void*)_activeBlock << endl;  
